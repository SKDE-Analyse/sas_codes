proc sgplot data=agg_alder_kjonn noautolegend noborder sganno=annoarsvar pad=(Bottom=5%);
    ...
    yaxis display=(noticks noline) values=&skala_alder label=&label_alder 
		labelattrs=(size=7 weight=bold) valuesformat=NLNUM8.0 valueattrs=(size=7);
	xaxis offsetmin=0.035 label='Alder' labelattrs=(size=7 weight=bold) valuesformat=NLNUM8.0 valueattrs=(size=7);
run;
ods graphics off;