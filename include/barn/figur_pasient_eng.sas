proc sgplot data=Pasientfigur noborder noautolegend sganno=ANNOARSVAR pad=(Bottom=5%) ;
     ...
     xaxis display=(nolabel) offsetmin=0.02 Min=0 values=&skala_pasient valuesformat=COMMA8.0 valueattrs=(size=7);
     inset (/*"(*ESC*){unicode'2605'x}" = "   2009" "(*ESC*){unicode'25a0'x}" = "   2010"*/ "(*ESC*){unicode'25a0'x}" = "   2011" 
                ...
                "(*ESC*){unicode'2212'x}(*ESC*){unicode'2212'x}"=" Norway") / position=bottomright textattrs=(size=7);
     format InnlPrPas COMMA8.2;
     format &ytable2 COMMA10.0;
run;
ods graphics off;