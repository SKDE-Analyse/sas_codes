...
proc sql;
   ...
   select *, sum(to_ant) as antall_to, sum (en_rate) AS rate_en, sum (to_rate) as rate_to
...
if bohf = 8888 then snittrate = ratesnitt;
	...
	keep Bohf type Plassering ratesnitt antall_to Innbyggere Andel en_ant tot_rate rate_en to_rate  labelpos No_snitt norge_snitt Mistext;
run;
...
hbarparm category=bohf response=andel / nooutline fillattrs=(color=CX95BDE6); 
 hbarparm category=bohf response=andel_no / nooutline) fillattrs=(color=CXC3C3C3);	
  ...
  xaxis label="Percentage"   labelattrs=(color=black size=7) offsetmin=0.02 valueattrs=(size=7);
