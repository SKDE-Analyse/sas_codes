...
hbarparm category=bohf response=pros_pos_no / baseline=1 fillattrs=(color=CXC3C3C3) barwidth=&soylebredde;	
    ...
    yaxis display=(noticks noline) label='Opptaksområde' labelattrs=(size=&fontst weight=bold) type=discrete discreteorder=data valueattrs=(size=&fontst);
   xaxis label="Forholdet mellom raten til aldersgruppen 50 - 74 og raten til aldersgruppen 75+ " type=linear labelattrs=(size=&fontst weight=bold) values=(0 to 2 by 0.1) offsetmin=0.05; 
