...
rename kontakttype=kontakttype_org kontakttypenavn=kontakttypenavn_org;
...
if lowcase(takstKode) in ('2ad','2ae','2ak','2aek','2ed','2fk','2af',
                      '074a','074ae','074b','074be','074d',
                      '2as','2at','2av','2aks','2akt','2akv','2aef','2afs','2aft','2afv') then nykontakttype=3; /*konsultasjon, beløp <100 - <1000*/
...
else if lowcase(takstKode) in ('1f','14','1j') then nykontakttype=5; /*tverrfaglig, beløp 100-1000*/

else if upcase(substr(takstKode,1,1))='L' or lowcase(takstKode) in ('5','616','h1','2kd') then nykontakttype=1;

else nykontakttype=0; /*ukjent*/ 
...
proc sql;
  ...
  select enkeltregning_lnr, max(nykontakttype) as kontakttype_kpr
  from takst
