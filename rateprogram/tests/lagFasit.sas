...
/*
Lagre anno-fil, for testing senere
*/
data skde_arn.test_rate_anno;
set anno;
run;
...
%let kommune=1; 	/*Bildefiler*/ %let Fig_AA_kom=; 	%let Fig_KI_kom=;
...
data skde_arn.test_rate_rv1;
set rv;
run;

data skde_arn.test_rate_andel1;
set andel;
run;

%omraadeNorge;

data skde_arn.test_rate_norge_agg;
set norge_agg;
run;

data skde_arn.test_rate_norge_agg_snitt;
set norge_agg_snitt;
run;
...
data skde_arn.test_rate_&Bo._AGG_CV;
set &Bo._AGG_CV;
run;

data skde_arn.test_rate_&Bo._Agg_rate;
set &Bo._Agg_rate;
run;
