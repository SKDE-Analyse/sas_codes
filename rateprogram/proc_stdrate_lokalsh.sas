...
Makro for å beregne rater for lokalsykehus.
Ratefigur skrives til angitt bildesti.
Makro gjør om til lokalsykehusområde basert på komnr og bydel. 

```
kortversjon (kjøres med default verdier for resten)
%proc_stdrate_lokalsh(dsn=, rate_var=, standardaar=, start=, slutt=, utdata=);
```
### Input
...
data xyz_rateutvalg;
set &dsn;
keep aar ermann alder komnr bydel &rate_var nyalder;
...
proc sql;
	create table xyz_rateutvalg as
	select a.*, b.bosh
	from xyz_rateutvalg a left join bo b
	on a.komnr=b.komnr
	and a.bydel=b.bydel;
 quit;
...
proc sql;
	...
	select aar, bosh, nyalder, ermann,
		sum(&rate_var) as antall
	from xyz_rateutvalg
	group by aar, bosh, nyalder, ermann;
quit;
...
proc sql;
	...
	select bosh, nyalder, ermann,
		sum(antall) as antall
	from xyz_ratedsn
	group by bosh, nyalder, ermann;
quit;
...
proc sql;
	create table xyz_pop as
	select a.*, b.bosh
	from xyz_pop a left join bo b
	on a.komnr=b.komnr
	and a.bydel=b.bydel;
 quit;
...
on a.bosh=b.bosh and a.aar=b.aar and a.nyalder=b.nyalder and a.ermann=b.ermann;
run;
...
proc sql;
	...
	from xyz_ratedata
	where aar=&standardaar and bosh=8888
	group by nyalder, ermann;
quit;
proc sql;
create table xyz_popN as
...
format aar aar_fmt. bosh bosh_fmt.;
run; 
...
proc tabulate data=xyz_tmp_rate;
	...
	CLASS aar bosh/	ORDER=UNFORMATTED MISSING;
	TABLE 
bosh={LABEL="" STYLE={NOBREAKSPACE=ON} STYLE(CLASSLEV)={NOBREAKSPACE=ON}},
...
data _null_;
set xyz_tmp_rate;
If Aar=9999 and bosh=8888 then do;
...
where Aar=9999 and bosh ne 8888;
var rate;
run;
...
data xyz_tmp_rater;
set xyz_tmp_rater;
if bosh=8888 then nrate=ratesnitt;
...
ODS Graphics ON /reset=All imagename="&rate_var._lokalsh_aarsvar" imagefmt=png border=off height=1000px;
...
hbarparm category=bosh response=ratesnitt / fillattrs=(color=CX95BDE6); 
hbarparm category=bosh response=nrate / fillattrs=(color=CXC3C3C3);
     /* Refline &NorgeSnitt / axis=x lineattrs=(Thickness=.5 color=Black pattern=2) name="Ref1"; */
%if &Antall_aar>1 %then %do; scatter x=rate&ar1 y=bosh / markerattrs=(symbol=circlefilled color=black size=5pt);%end;
%if &Antall_aar>2 %then %do; scatter x=rate&ar2 y=bosh / markerattrs=(symbol=circlefilled color=grey  size=7pt); %end;
%if &Antall_aar>3 %then %do; scatter x=rate&ar3 y=bosh / markerattrs=(symbol=circle       color=black size=9pt);%end;
%if &Antall_aar>4 %then %do; scatter x=rate&ar4 y=bosh / markerattrs=(symbol=circle       color=red   size=9pt);%end;
%if &Antall_aar>5 %then %do; scatter x=rate&ar5 y=bosh / markerattrs=(symbol=circle       color=red   size=11pt);%end;
%if &Antall_aar>6 %then %do; scatter x=rate&ar6 y=bosh / markerattrs=(symbol=circle       color=black size=11pt);%end;

Highlow Y=bosh low=Min high=Max / type=line name="hl2" lineattrs=(color=black thickness=1 pattern=1);
