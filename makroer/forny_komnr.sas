...
data forny (keep=komnr_work nykom);
  set forny_komnr(rename=(ny_komnr=nykom gml_komnr=komnr_work));
run;

/* Prepare input with original and working komnr */
data data_forny_input;
  set &inndata;
  komnr_work = &kommune_nr. + 0;
run;
...
data &inndata(drop=rc prev_komnr nykom);
  ...
  do until (prev_komnr = komnr_work or komnr_work = .);
    prev_komnr = komnr_work;
    rc = h.find();
    if rc = 0 and nykom > 0 then komnr_work = nykom;
  end;
komnr = komnr_work; /*skriv ut ny komnr*/ 
drop komnr_work; /*drop hjelpevariabel komnr_work*/
run;
